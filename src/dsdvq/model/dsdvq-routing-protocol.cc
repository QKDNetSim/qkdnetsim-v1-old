...
NS_LOG_COMPONENT_DEFINE ("DsdvqRoutingProtocol");

namespace dsdvq {

NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for DSDVQ control traffic
const uint32_t RoutingProtocol::DSDVQ_PORT = 69;

/// Tag used by DSDVQ implementation
struct DeferredRouteOutputTag : public Tag
{
  ...
  GetTypeId ()
  {
    static TypeId tid = TypeId ("ns3::dsdvq::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("Dsdvq")
...
{
  static TypeId tid = TypeId ("ns3::dsdvq::RoutingProtocol")
...
void
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> streamWrapper, std::string additionalData) const
{

  NS_LOG_FUNCTION (this << Simulator::Now ().GetNanoSeconds() << additionalData); 

  std::ostream *stream = streamWrapper->GetStream (); 
  *stream << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()  << "\t"
        << " Time: " << Simulator::Now ().GetSeconds ()  << "\t"
        << "s " << "\t" << additionalData << std::endl; 

  m_routingTable.Print (streamWrapper);
}
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No dsdvq interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             ErrorCallback ecb)
{
  ...
  if (m_socketAddresses.empty ())
    {
      NS_LOG_DEBUG ("No dsdvq interfaces");
      return false;
    }
  NS_ASSERT (m_ipv4 != 0);
  ...
  // DSDVQ is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      NS_LOG_FUNCTION(this << "DSDVQ is not a multicast routing protocol");
      return false;
    }
  ...
  NS_LOG_FUNCTION(this << "DSDVQ 438");
  ...
  NS_LOG_FUNCTION(this << "DSDVQ 448");

  // LOCAL DELIVARY TO DSDVQ INTERFACES
              ...
              if (lcb.IsNull () == false)
                {
                  ...
                  NS_LOG_FUNCTION(this << "Before lcb");
                  lcb (p, header, iif);
                  // Fall through to additional processing
                  NS_LOG_FUNCTION(this << "After lcb");
                }
              else
                {
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when DSDVQ does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // DSDVQ needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available DSDVQ interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid DSDVQ source address not found");
...
void
RoutingProtocol::RecvDsdvq (Ptr<Socket> socket)
{
  Address sourceAddress;
  Ptr<Packet> advpacket = Create<Packet> ();
  advpacket = m_ipv4->GetObject<Node> ()->GetObject<QKDManager> ()->MarkEncrypt (advpacket);
  ...
  NS_LOG_FUNCTION (m_mainAddress << " received dsdvq packet of size: " << packetSize
                                 << " and packet id: " << packet->GetUid ());
  uint32_t count = 0;
  for (; packetSize > 0; packetSize = packetSize - 12)
    {
      count = 0;
      DsdvqHeader dsdvqHeader, tempDsdvqHeader;
      packet->RemoveHeader (dsdvqHeader);
      NS_LOG_DEBUG ("Processing new update for " << dsdvqHeader.GetDst ());
      ...
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j
           != m_socketAddresses.end (); ++j)
        {
          ...
          if (dsdvqHeader.GetDst () == interface.GetLocal ())
            {
              if (dsdvqHeader.GetDstSeqno () % 2 == 1)
                {
                  NS_LOG_DEBUG ("Sent Dsdvq update back to the same Destination, "
      ...
      NS_LOG_DEBUG ("Received a DSDVQ packet from "
                    << sender << " to " << receiver << ". Details are: Destination: " << dsdvqHeader.GetDst () << ", Seq No: "
                    << dsdvqHeader.GetDstSeqno () << ", HopCount: " << dsdvqHeader.GetHopCount ());
      RoutingTableEntry fwdTableEntry, advTableEntry;
      EventId event;
      bool permanentTableVerifier = m_routingTable.LookupRoute (dsdvqHeader.GetDst (),fwdTableEntry);
      if (permanentTableVerifier == false)
          ...
          if (dsdvqHeader.GetDstSeqno () % 2 != 1)
            {
              ...
              RoutingTableEntry newEntry (
                /*device=*/ dev, /*dst=*/
                dsdvqHeader.GetDst (), /*seqno=*/
                dsdvqHeader.GetDstSeqno (),
                /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                /*hops=*/ dsdvqHeader.GetHopCount (), /*next hop=*/
                sender, /*lifetime=*/
        ...
        {
          if (!m_advRoutingTable.LookupRoute (dsdvqHeader.GetDst (),advTableEntry))
            {
              ...
              m_advRoutingTable.LookupRoute (dsdvqHeader.GetDst (),advTableEntry);
            }
          if (dsdvqHeader.GetDstSeqno () % 2 != 1)
            {
              if (dsdvqHeader.GetDstSeqno () > advTableEntry.GetSeqNo ())
                {
                  // Received update with better seq number. Clear any old events that are running
                  if (m_advRoutingTable.ForceDeleteIpv4Event (dsdvqHeader.GetDst ()))
                    {
                  ...
                  if (dsdvqHeader.GetHopCount () != advTableEntry.GetHop ())
                    {
                      advTableEntry.SetSeqNo (dsdvqHeader.GetDstSeqno ());
                      ...
                      advTableEntry.SetHop (dsdvqHeader.GetHopCount ());
                      NS_LOG_DEBUG ("Received update with better sequence number and changed metric.Waiting for WST");
                      Time tempSettlingtime = GetSettlingTime (dsdvqHeader.GetDst ());
                      ...
                      m_advRoutingTable.AddIpv4Event (dsdvqHeader.GetDst (),event);
              ...
              else if (dsdvqHeader.GetDstSeqno () == advTableEntry.GetSeqNo ())
                {
                  if (dsdvqHeader.GetHopCount () < advTableEntry.GetHop ())
                    {
                      ...
                      m_advRoutingTable.ForceDeleteIpv4Event (dsdvqHeader.GetDst ());
                      advTableEntry.SetSeqNo (dsdvqHeader.GetDstSeqno ());
                      advTableEntry.SetLifeTime (Simulator::Now ());
                      advTableEntry.SetFlag (VALID);
                      advTableEntry.SetEntriesChanged (true);
                  ...
                  else
                    {
                       ...
                       * Discard that update.
                       */
                      if (!m_advRoutingTable.AnyRunningEvent (dsdvqHeader.GetDst ()))
                        {
                      ...
                      NS_LOG_DEBUG ("Received update with same seq number and "
                                    "same/worst metric for, " << dsdvqHeader.GetDst () << ". Discarding the update.");
                    }
                }
              else
                {
                  ...
                  if (!m_advRoutingTable.AnyRunningEvent (dsdvqHeader.GetDst ()))
                    {
                      m_advRoutingTable.DeleteRoute (dsdvqHeader.GetDst ());
                    }
                  NS_LOG_DEBUG (dsdvqHeader.GetDst () << " : Received update with old seq number. Discarding the update.");
                }
            }
          else
            {
              NS_LOG_DEBUG ("Route with infinite metric received for "
                            << dsdvqHeader.GetDst () << " from " << sender);
              ...
              if (sender == advTableEntry.GetNextHop ())
                {
                  ...
                  m_routingTable.GetListOfDestinationWithNextHop (dsdvqHeader.GetDst (),dstsWithNextHopSrc);
                  m_routingTable.DeleteRoute (dsdvqHeader.GetDst ());
                  advTableEntry.SetSeqNo (dsdvqHeader.GetDstSeqno ());
                  advTableEntry.SetEntriesChanged (true);
                  m_advRoutingTable.Update (advTableEntry);
              ...
              else
                {
                  ...
                  NS_LOG_DEBUG (dsdvqHeader.GetDst () <<
...
{
  this->PrintRoutingTable (routingStream, "SendTriggeredUpdate");
      ...
      for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i = allRoutes.begin (); i != allRoutes.end (); ++i)
        {
          ...
          if ((i->second.GetEntriesChanged () == true) && (!m_advRoutingTable.AnyRunningEvent (temp.GetDestination ())))
            {
              dsdvqHeader.SetDst (i->second.GetDestination ());
              dsdvqHeader.SetDstSeqno (i->second.GetSeqNo ());
              dsdvqHeader.SetHopCount (i->second.GetHop () + 1);
              temp.SetFlag (VALID);
      ...
      if (packet->GetSize () >= 12)
        {
          ...
          //dsdvqHeader.SetDst (m_ipv4->GetAddress (1, 0).GetLocal ());
          dsdvqHeader.SetDst (iface.GetLocal ());
          dsdvqHeader.SetDstSeqno (temp2.GetSeqNo ());
          dsdvqHeader.SetHopCount (temp2.GetHop () + 1);
          ...
          qkdCommandTag.SetRoutingProtocolNumber(DSDVQ_PACKET_HEADER_PROTOCOL_NUMBER);
          ...
          socket->SendTo (packet, 0, InetSocketAddress (destination, DSDVQ_PORT));
...
uint32_t 
RoutingProtocol::FindLocalInterfaceForDestination(Ipv4Address dst){
        
  NS_LOG_FUNCTION(this << dst);

  for(uint32_t i=1; i<m_ipv4->GetNInterfaces(); i++){ 
    //if(m_ipv4->GetAddress (i,0).GetBroadcast() == dst || m_ipv4->GetAddress (i,0).GetLocal() == dst){
    if(
       dst.CombineMask( m_ipv4->GetAddress (i,0).GetMask() ) 
       ==  m_ipv4->GetAddress (i,0).GetLocal().CombineMask( m_ipv4->GetAddress (i,0).GetMask() ) 
    ){
        NS_LOG_FUNCTION( "Node: " << m_ipv4->GetObject<Node> ()->GetId () 
                                  << "LocalIP from device:" 
                                  << m_ipv4->GetAddress (i,0).GetLocal()
                                  << "\tDestination:" << dst);
        return i; 
    }
  }

  //if nothing is not found then return the default interface
  return 1;
}
...
{
  this->PrintRoutingTable (routingStream, "SendPeriodicUpdate");
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j
       != m_socketAddresses.end (); ++j)
    {
      ...
      NS_LOG_FUNCTION(this << "Sending periodic update for interface " << iface.GetLocal());
      ...
      for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i = allRoutes.begin (); i != allRoutes.end (); ++i)
        {
          ...
          if (i->second.GetHop () == 0)
            {
              uint32_t localInterface = FindLocalInterfaceForDestination(i->first);

              RoutingTableEntry ownEntry;
              //dsdvqHeader.SetDst (m_ipv4->GetAddress (1,0).GetLocal ());
              dsdvqHeader.SetDst (m_ipv4->GetAddress (localInterface,0).GetLocal ());
              dsdvqHeader.SetDstSeqno (i->second.GetSeqNo () + 2);
              dsdvqHeader.SetHopCount (i->second.GetHop () + 1);
              m_routingTable.LookupRoute (m_ipv4->GetAddress (localInterface,0).GetBroadcast (),ownEntry);
              ownEntry.SetSeqNo (dsdvqHeader.GetDstSeqno ());
          ...
          NS_LOG_DEBUG ("Forwarding details are, Destination: " << dsdvqHeader.GetDst ()
                                                                << ", SeqNo:" << dsdvqHeader.GetDstSeqno ()
                                                                << ", HopCount:" << dsdvqHeader.GetHopCount ()
      ...
      socket->SendTo (packet, 0, InetSocketAddress (destination, DSDVQ_PORT));
      NS_LOG_FUNCTION ("PeriodicUpdate Packet UID is : " << packet->GetUid () << " destination " << destination );
    }
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  temp << "dsdvq-routing-tables_" << m_ipv4->GetObject<Node>()->GetId() << ".txt"; 
...
{
  this->PrintRoutingTable (routingStream, "NotifyInterfaceUp");
  ...
  Ptr<Socket> socket = Socket::CreateSocket (GetObject<Node> (),VirtualUdpSocketFactory::GetTypeId ());
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvDsdvq,this)); 
  InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), DSDVQ_PORT); 
  //socket->SetIpTos(16); //16 - band 0; 2 - band 1 
  if (socket->Bind (inetAddr))
  {
   NS_FATAL_ERROR ("Failed to bind() DSDVQ socket");
  }
  ...
  // Create a socket to listen on broadcast
  // From ns3.27 : "Sockets do not receive anymore broadcast packets, unless they are bound to an "Any" address (0.0.0.0) or to a subnet-directed broadcast packet (e.g., x.y.z.0 for a /24 noterok)."
  Ptr<Socket> socket2 = Socket::CreateSocket (GetObject<Node> (),VirtualUdpSocketFactory::GetTypeId ());
  NS_ASSERT (socket2 != 0);
  socket2->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvDsdvq,this)); 
  InetSocketAddress inetAddr2 (m_ipv4->GetAddress (i, 0).GetBroadcast (), DSDVQ_PORT); 
  //socket->SetIpTos(16); //16 - band 0; 2 - band 1 
  if (socket2->Bind (inetAddr2))
  {
   NS_FATAL_ERROR ("Failed to bind() DSDVQ socket");
  }
  socket2->BindToNetDevice (m_ipv4->GetNetDevice (i)); 
  socket2->SetAttribute ("IpTtl",UintegerValue (1));
  socket2->SetAllowBroadcast (true);
  socket2->SetRecvPktInfo (true);
  socket2->ShutdownSend ();
  m_socketAddresses.insert (std::make_pair (socket2,iface));
...
{
  this->PrintRoutingTable (routingStream, "NotifyAddAddress");
      ...
      if (l3->GetNAddresses (i))
        {
          ...
          socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), DSDVQ_PORT));
          socket->SetAllowBroadcast (true);
          m_socketAddresses.insert (std::make_pair (socket,iface));
        }
    }
