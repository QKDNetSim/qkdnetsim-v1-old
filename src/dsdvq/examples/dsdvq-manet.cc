...
NS_LOG_COMPONENT_DEFINE ("DsdvqManetExample");

/**
 * \ingroup dsdvq
 * \ingroup dsdvq-examples
 * \ingroup examples
 *
 * \brief DSDVQ Manet example
 */
class DsdvqManetExample
{
public:
  DsdvqManetExample ();
  /**
...
int main (int argc, char **argv)
{
  ...
  std::string CSVfileName = "DsdvqManetExample.csv";
  ...
  cmd.AddValue ("CSVfileName", "The name of the CSV output file name[Default:DsdvqManetExample.csv]", CSVfileName);
  cmd.Parse (argc, argv);
...
DsdvqManetExample::DsdvqManetExample ()
  : bytesTotal (0),
    packetsReceived (0)
{
}
...
void
DsdvqManetExample::ReceivePacket (Ptr <Socket> socket)
{
...
void
DsdvqManetExample::CheckThroughput ()
{
  ...
  out.close ();
  packetsReceived = 0;
  Simulator::Schedule (Seconds (1.0), &DsdvqManetExample::CheckThroughput, this);
}

Ptr <Socket>
DsdvqManetExample::SetupPacketReceive (Ipv4Address addr, Ptr <Node> node)
{
  ...
  sink->Bind (local);
  sink->SetRecvCallback (MakeCallback ( &DsdvqManetExample::ReceivePacket, this));

  return sink;
}

void
DsdvqManetExample::CaseRun (uint32_t nWifis, uint32_t nSinks, double totalTime, std::string rate,
                           ...
                           double dataStart, bool printRoutes, std::string CSVfileName)
{
  ...
  std::string tr_name = "Dsdvq_Manet_" + t_nodes + "Nodes_" + sTotalTime + "SimTime";
...
void
DsdvqManetExample::SetupMobility ()
{
...
void
DsdvqManetExample::CreateDevices (std::string tr_name)
{
...
void
DsdvqManetExample::InstallInternetStack (std::string tr_name)
{
  DsdvqHelper dsdvq;
  dsdvq.Set ("PeriodicUpdateInterval", TimeValue (Seconds (m_periodicUpdateInterval)));
  dsdvq.Set ("SettlingTime", TimeValue (Seconds (m_settlingTime)));
  InternetStackHelper stack;
  stack.SetRoutingHelper (dsdvq); // has effect on the next Install ()
  stack.Install (nodes);
  ...
  if (m_printRoutes)
    {
      ...
      dsdvq.PrintRoutingTableAllAt (Seconds (m_periodicUpdateInterval), routingStream);
    }
}

void
DsdvqManetExample::InstallApplications ()
{
