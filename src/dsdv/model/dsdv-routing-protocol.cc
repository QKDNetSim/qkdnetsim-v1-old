...
const uint32_t RoutingProtocol::DSDV_PORT = 69;
                             ...
                             ErrorCallback ecb)
{
  ...
  if (dst.IsMulticast ())
    {
      NS_LOG_FUNCTION(this << "DSDV is not a multicast routing protocol");
      return false;
    }
      ...
      if (packet->GetSize () >= 12)
        {
          ...
          qkdCommandTag.SetRoutingProtocolNumber(DSDV_PACKET_HEADER_PROTOCOL_NUMBER);
          ...
          if (i->second.GetHop () == 0)
            {
              ...
              //dsdvHeader.SetDst (m_ipv4->GetAddress (1,0).GetLocal ());
              dsdvHeader.SetDst (m_ipv4->GetAddress (localInterface,0).GetLocal ());
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  temp << "dsdv-routing-tables_" << m_ipv4->GetObject<Node>()->GetId() << ".txt"; 
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), DSDV_PORT); 
  //socket->SetIpTos(16); //16 - band 0; 2 - band 1 
  if (socket->Bind (inetAddr))
  {
   NS_FATAL_ERROR ("Failed to bind() DSDV socket");
  }
  ...
  Ptr<Socket> socket2 = Socket::CreateSocket (GetObject<Node> (),UdpSocketFactory::GetTypeId ());
  NS_ASSERT (socket2 != 0);
  socket2->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvDsdv,this)); 
  InetSocketAddress inetAddr2 (m_ipv4->GetAddress (i, 0).GetBroadcast (), DSDV_PORT); 
  //socket->SetIpTos(16); //16 - band 0; 2 - band 1 
