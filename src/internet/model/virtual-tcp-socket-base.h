 ...
 */
#ifndef VIRTUAL_TCP_SOCKET_BASE_H
#define VIRTUAL_TCP_SOCKET_BASE_H
 ...
 * \ingroup tcp
 *
 * \brief A base class for implementation of a Overlay (virtual) stream socket using TCP.
 *
 ...
 * of TcpCongestionOps. Part of VirtualTcpSocketBase is modified from the original
 ...
 * VirtualTcpSocketBase. In particular, each congestion control is now a subclass of
 * the main TcpCongestionOps class. Switching between congestion algorithm is
 * now a matter of setting a pointer into the VirtualTcpSocketBase class. The idea
 ...
 * used, which consists in chaining callbacks from TcpSocketState to VirtualTcpSocketBase
 ...
 *
 */
class VirtualTcpSocketBase : public TcpSocket
{
public:
  /**
   * Get the type ID.
   ...
   */
  VirtualTcpSocketBase (const VirtualTcpSocketBase& sock);
  virtual ~VirtualTcpSocketBase (void);

  // Set associated Node, VirtualTcpL4Protocol, RttEstimator to this socket
   ...
   */
  virtual void SetTcp (Ptr<VirtualTcpL4Protocol> tcp);
  ...
  virtual int Bind (void);    // Bind a socket by setting up endpoint in VirtualTcpL4Protocol
  virtual int Bind6 (void);    // Bind a socket by setting up endpoint in VirtualTcpL4Protocol
  ...
  typedef void (* TcpTxRxTracedCallback)(const Ptr<const Packet> packet, const TcpHeader& header,
                                         const Ptr<const VirtualTcpSocketBase> socket);
...
protected:
  ...
  /**
   * \brief Called by VirtualTcpSocketBase::ForwardUp{,6}().
   *
...
protected:
  ...
  Ptr<VirtualTcpL4Protocol>  m_tcp;                 //!< the associated TCP L4 protocol
  ...
  TracedCallback<Ptr<const Packet>, const TcpHeader&,
                 Ptr<const VirtualTcpSocketBase> > m_txTrace; //!< Trace of transmitted packets

  TracedCallback<Ptr<const Packet>, const TcpHeader&,
                 Ptr<const VirtualTcpSocketBase> > m_rxTrace; //!< Trace of received packets
