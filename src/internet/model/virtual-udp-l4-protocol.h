...
#ifndef VIRTUAL_UDP_L4_PROTOCOL_H
#define VIRTUAL_UDP_L4_PROTOCOL_H
...
class Ipv6EndPoint;
class VirtualUdpSocketImpl;

/**
 * \ingroup internet
 * \defgroup udp UDP
 *
 * This  is  an  implementation of the Overlay (virtual) User Datagram Protocol described in
 ...
 */
class VirtualUdpL4Protocol : public IpL4Protocol {
public:
  /**
  ...
  VirtualUdpL4Protocol ();
  virtual ~VirtualUdpL4Protocol ();
...
private:
   ...
   */
  VirtualUdpL4Protocol (const VirtualUdpL4Protocol &);
  /**
   * \brief Copy constructor
   *
   * Defined and not implemented to avoid misuse
   * \returns
   */
  VirtualUdpL4Protocol &operator = (const VirtualUdpL4Protocol &);

  std::vector<Ptr<VirtualUdpSocketImpl> > m_sockets;      //!< list of sockets
...
#endif /* VIRTUAL_UDP_L4_PROTOCOL_H */
