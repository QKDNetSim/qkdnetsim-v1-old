 ...
 * \ingroup tests
 *
 * \brief AODVQ regression test suite
 */
class AodvqRegressionTestSuite : public TestSuite
{
public:
  AodvqRegressionTestSuite () : TestSuite ("routing-aodvq-regression", SYSTEM)
  {
    SetDataDir (NS_TEST_SOURCEDIR);
    // General RREQ-RREP-RRER test case
    AddTestCase (new ChainRegressionTest ("aodvq-chain-regression-test"), TestCase::QUICK);
...
} g_aodvqRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest (const char * const prefix, Time t, uint32_t size, Time arpAliveTimeout)
  : TestCase ("AODVQ chain regression test"),
    m_nodes (0),
    m_prefix (prefix),
    m_time (t),
    m_size (size),
...
ChainRegressionTest::CreateDevices ()
{
  ...
  // 2. Setup TCP/IP & AODVQ
  AodvqHelper aodvq; // Use default parameters here
  InternetStackHelper internetStack;
  internetStack.SetRoutingHelper (aodvq);
  ...
  streamsUsed += aodvq.AssignStreams (*m_nodes, streamsUsed);
  // AODVQ uses m_size more streams
