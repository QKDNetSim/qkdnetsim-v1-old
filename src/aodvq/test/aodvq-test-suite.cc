 ...
 */
#include "ns3/test.h"
#include "ns3/aodvq-neighbor.h"
#include "ns3/aodvq-packet.h"
#include "ns3/aodvq-rqueue.h"
#include "ns3/aodvq-rtable.h"
...
{
  TypeHeaderTest () : TestCase ("AODVQ TypeHeader")
  {
  }
  virtual void DoRun ()
  {
    TypeHeader h (AODVQTYPE_RREQ);
    NS_TEST_EXPECT_MSG_EQ (h.IsValid (), true, "Default header is valid");
    NS_TEST_EXPECT_MSG_EQ (h.Get (), AODVQTYPE_RREQ, "Default header is RREQ");

    Ptr<Packet> p = Create<Packet> ();
    p->AddHeader (h);
    TypeHeader h2 (AODVQTYPE_RREP);
...
{
  RreqHeaderTest () : TestCase ("AODVQ RREQ")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepHeaderTest () : TestCase ("AODVQ RREP")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepAckHeaderTest () : TestCase ("AODVQ RREP-ACK")
  {
  }
  virtual void DoRun ()
  {
...
{
  RerrHeaderTest () : TestCase ("AODVQ RERR")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for AODVQ routing table entry
 */
...
struct AodvqRqueueTest : public TestCase
{
  AodvqRqueueTest () : TestCase ("Rqueue"),
                      q (64, Seconds (30))
  {
  }
  virtual void DoRun ();
...
AodvqRqueueTest::DoRun ()
{
  ...
  h.SetDestination (Ipv4Address ("1.2.3.4"));
  h.SetSource (Ipv4Address ("4.3.2.1"));
  Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback (&AodvqRqueueTest::Unicast, this);
  Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback (&AodvqRqueueTest::Error, this);
  ...
  Simulator::Schedule (q.GetQueueTimeout () + Seconds (1), &AodvqRqueueTest::CheckTimeout, this);
...
void
AodvqRqueueTest::CheckSizeLimit ()
{
 ...
 * \brief Unit test for AODVQ routing table entry
 */
struct AodvqRtableEntryTest : public TestCase
{
  AodvqRtableEntryTest () : TestCase ("RtableEntry")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for AODVQ routing table
 */
struct AodvqRtableTest : public TestCase
{
  AodvqRtableTest () : TestCase ("Rtable")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief AODVQ test suite
 */
class AodvqTestSuite : public TestSuite
{
public:
  AodvqTestSuite () : TestSuite ("routing-aodvq", UNIT)
  {
    ...
    AddTestCase (new AodvqRqueueTest, TestCase::QUICK);
    AddTestCase (new AodvqRtableEntryTest, TestCase::QUICK);
    AddTestCase (new AodvqRtableTest, TestCase::QUICK);
  }
} g_aodvqTestSuite; ///< the test suite
