...
Ad Hoc On-Demand Distance Vector (AODVQ)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (AODVQ) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 AODVQ model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the AODVQ-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the AODVQ model lives in the directory `src/aodvq`.

Design
++++++

Class ``ns3::aodvq::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::aodvq::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::aodvq::RouteInput``,
...
this functionality are attributes of ``ns3::aodvq::RoutingProtocol``. 
...
AODVQ discovers routes on demand.  Therefore, the AODVQ model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in aodvq-rqueue.cc. A smart pointer to 
...
* This AODVQ implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from AODVQ-UU implementation and can be disabled.
...
the assertion from the AODVQ RFC that AODVQ works over UDP.  This model uses
